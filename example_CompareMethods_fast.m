...
%
% Fast Linear Radon Transform methods:
% - L2 (Schultz, 2012)
% - CGsimple (Ji, 2006)
% - CGG_weight (Ji, 2006) *This one seems to work the best on real data
% - CGhestenes (Ji, 2006; Claerbout, 1992)
% 
% J. Russell
...
%%
% Plot figures. Normalize each frequency separately for plotting purposes.
figure(3); clf;
...
ylim([v_min v_max]);
title('Schultz & Gu'); ylabel('Velocity (km/s)'); xlabel('Period (s)');
set(gca,'YDir','normal');
...
xlim([min(perplot(1,1:end)) max(perplot(1,1:end))]);
ylim([v_min v_max]);
title('CG simple'); ylabel('Velocity (km/s)'); xlabel('Period (s)');
set(gca,'YDir','normal');
...
xlim([min(perplot(1,1:end)) max(perplot(1,1:end))]);
ylim([v_min v_max]);
title('CGG weight'); ylabel('Velocity (km/s)'); xlabel('Period (s)');
set(gca,'YDir','normal');
...
xlim([min(perplot(1,1:end)) max(perplot(1,1:end))]);
ylim([v_min v_max]);
title('CG Hestenes'); ylabel('Velocity (km/s)'); xlabel('Period (s)');
set(gca,'YDir','normal');
